...
export class RingSignature {
    ...
    private generateChain(message: Uint8Array, L: ECPoint): Uint8Array { const hash = sha3_256.create(); hash.update(message); hash.update(this.hexToBytes(L.getX().toString('hex'))); hash.update(this.hexToBytes(L.getY().toString('hex'))); return new Uint8Array(hash.digest()); }

    // Helper to convert hex string to Uint8Array
    private hexToBytes(hex: string): Uint8Array { const bytes = new Uint8Array(hex.length / 2); for (let i = 0; i < hex.length; i += 2) { bytes[i / 2] = parseInt(hex.slice(i, i + 2), 16); } return bytes; }

    async create(privateKey: PrivateKey, publicKeys: PublicKey[], message: Uint8Array, caseIdentifier?: Uint8Array): Promise<Signature> {
    ...
    private generateKeyImage(privateKey: PrivateKey): { x: string; y: string } {
        const hexToBytes = (hex: string): Uint8Array => { const bytes = new Uint8Array(hex.length / 2); for (let i = 0; i < hex.length; i += 2) { bytes[i / 2] = parseInt(hex.slice(i, i + 2), 16); } return bytes; };
    ...
    private hashPoints(points: ECPoint[]): Uint8Array {
        const hash = sha3_256.create(); const hexToBytes = (hex: string): Uint8Array => { const bytes = new Uint8Array(hex.length / 2); for (let i = 0; i < hex.length; i += 2) { bytes[i / 2] = parseInt(hex.slice(i, i + 2), 16); } return bytes; };
        points.forEach(p => {
    ...
    private hashToCurve(input: Uint8Array): ECPoint { const hash = sha3_256.create(); hash.update(input); const hashValue = new BN(hash.digest()); return this.curve.g.mul(hashValue); }
}